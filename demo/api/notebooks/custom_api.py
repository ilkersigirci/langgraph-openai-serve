...
def _(mo):
    ...
    Start the demo with `just demo/api --editable`. These examples
    use the deterministic custom-schema graph, so no upstream model key is
    needed. Each call is stateless: send `store=False` and carry any history in
    `input`.
    """)
    return
