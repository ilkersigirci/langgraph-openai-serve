...
- `runnable_config` contains callbacks and execution identity. For an
  interrupt-enabled or background graph, LGOS derives
  ...
  `metadata.lgos_run_id`. When callbacks are configured, LGOS also adds a
  ...
  to callbacks during checkpointed execution.
...
=== "Background response"
    ...
    When `background=true`, the HTTP request persists the prepared OpenAI
    envelope and returns without invoking the graph. A separate
    `BackgroundWorker` resolves the same `GraphConfig` and enters
    `run_background_graph()`. It inspects the checkpoint before every attempt,
    applies graph input only when no checkpoint exists, and otherwise resumes
    unfinished work with `None` input. Execution uses
    `graph.ainvoke(version="v2", durability="sync")`.
