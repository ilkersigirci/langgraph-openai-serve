...
- `demo/ui/openwebui/src/lgos_openwebui/functions/uservalves_simple.py` is a
  small single-model Pipe for demonstrating
...
The sync command signs in through `/api/v1/auths/signin` with the Compose admin
credentials and creates or updates only the two bundled Functions.
...
`demo/ui/openwebui/src/lgos_openwebui/functions/`; unchanged Functions are
skipped.
...
The defaults match `compose.yaml`. Override them for another local Open
WebUI instance with `DEMO_OPENWEBUI_URL`, `DEMO_OPENWEBUI_ADMIN_EMAIL`, and
`DEMO_OPENWEBUI_ADMIN_PASSWORD`. Set the password in the environment rather
