...
| [`background-report-agent`](background-report-agent.md) | Deterministic background execution recovered through checkpoints in an independently deployed worker, with no model call | `background` | Hatchet, PostgreSQL checkpointer, Response store, and run coordinator |
...
different: each owns one bounded, polling-only Response lifecycle and its
recovery checkpoint. It still does not persist a UI
